...
// SQLite database path and directory setup (only if using SQLite)
const dbDir = path.join(process.cwd(), 'data');
const sqliteDbPath = path.join(dbDir, 'choretracker.db');

// Only create the data directory if we're using SQLite
if (!usePostgres) {
  try {
    // Make sure the data directory exists for SQLite
    if (!fs.existsSync(dbDir)) {
      console.log(`Creating SQLite data directory: ${dbDir}`);
      fs.mkdirSync(dbDir);
    }
  } catch (error) {
    console.error(`Error creating data directory: ${error.message}`);
    // Don't throw here - we'll let the database operations fail if needed
  }
}
  ...
  if (usePostgres) {
    ...
    if (!fs.existsSync(dbDir)) {
      try {
        ...
        throw new Error(`Cannot create SQLite data directory: ${error.message}`);
      }
    }
